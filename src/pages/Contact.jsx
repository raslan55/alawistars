...
const ContactPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setTimeout(() => setStatus(""), 3000); // يخفي الرسالة بعد 3 ثواني
    } catch (error) {
          ...
          <form onSubmit={handleSubmit} className="md:w-1/2 space-y-4">
            ...
            {(status === "success" || status === "error") && (
              <motion.div
                ...
                className={`mt-4 px-3 py-2 rounded shadow text-white font-medium text-center ${
                  status === "success" ? "bg-green-500" : "bg-red-500"
                }`}
              >
                {status === "success"
                  ? t("Message_Sent_Successfully")
                  : t("Message_Sending_Failed")}
              </motion.div>
            )}
